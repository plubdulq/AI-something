...
with col2:
    ...
    num_givens = difficulty_levels.get(st.session_state.difficulty, 25)  # Default to medium

    st.markdown(f"""
    <h6>Initial Sudoku : <span style='color: #FF5733; font-size: 1.2em; font-weight: bold;'> 
        </span>
    </h6>
    """, unsafe_allow_html=True)
    st.markdown(f"""
    <h6> <span style='color: #FF5733; font-size: 1.2em; font-weight: bold;'> 
            {st.session_state.difficulty.capitalize()} ({num_givens} givens)
        </span>
    </h6>
    """, unsafe_allow_html=True)
...
with b2:
    # Determine if the resume button should be disabled
    comparison_finished = (
        st.session_state.current_step_blind >= len(st.session_state.steps_blind) and 
    ...
    # Disable button if comparison is finished
    if st.button(auto_play_label, disabled=comparison_finished):
        st.session_state.auto_play = not st.session_state.auto_play
        st.rerun()

with b3:
    if st.button("New board"):
        # Generate new board with the selected difficulty
        st.session_state.difficulty = difficulty
        st.session_state.original_board = generate_sudoku(difficulty)
        ...
        st.session_state.current_step_blind = 0
        st.session_state.current_step_heuristic = 0
        st.session_state.solving = True  # Start solving immediately
        st.session_state.auto_play = True  # Auto-play visualization
        st.session_state.highlight_pos_blind = None
        st.session_state.highlight_pos_heuristic = None
        st.session_state.memory_blind = []
        st.session_state.memory_heuristic = []

        # Start solving the new board immediately
        start_time = time.time()
        solve_sudoku_blind(st.session_state.board_blind, st.session_state.steps_blind)
        st.session_state.time_blind = time.time() - start_time
